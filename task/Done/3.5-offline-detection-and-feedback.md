...
- Created `apps/pictho-app/src/offline/useOnlineStatus.ts` — a custom React hook that
...
- Created `apps/pictho-app/src/offline/OfflineIndicator.tsx` — a subtle fixed-
...
**Reorganisation (PR feedback)**:

- Files reorganised from type-based folders (`components/`, `hooks/`) to the feature-
  based `offline/` folder as per the project convention.
- `serviceWorkerRegistration.ts` moved from `src/` root to `src/offline/`.
- Download workers moved from `src/workers/` into `src/firstLaunch/` (the feature that owns them).
- TypeScript types moved from `src/types/` to `src/shared/types/` (shared across features).
- COPILOT-AGENT-GUIDE.md and PROJECT-OVERVIEW.md updated with folder-structure convention.
- All Todo task files updated with `## File Location` guidance.
